...
// Picks task by the highest priority time.
Task* pickNextTask()
{
        if (head)
        {
                Task* maiorpriority = head->task;                
                struct node* curr = head;
                while (curr->next) // While not last element
                {
                        curr = curr->next; // Skip to the next
                        if (curr->task->priority > maiorpriority->priority) 
                                maiorpriority = curr->task;
                }
                return maiorpriority;
        }
...
void schedule()
{
        ...
        FILE* fp = fopen("priority.txt","w"); // File output
