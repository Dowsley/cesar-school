...
// Utility function for task list sorting.
void swapTask(struct node* x, struct node* y)
{
    struct task* temp = x->task; 
    x->task = y->task; 
    y->task = temp;
}
...
void schedule()
{
        ...
        FILE* fp = fopen("priority_rr.txt","w"); // File output
        fprintf(fp,"EXECUTION BY PRIORITY RR\n");
